...
#define LOG_LEVEL 5
// 5 ERROR 6 NOTICE 7 INFO 8 DEBUG
int log_level = LOG_LEVEL; ////FIXME

#define CRCPOLY_LE 0xedb88320
#define LOSS_RECOVERY_ENABLE 1
#define CC_ENABLE 1
#define LOSS_CC_ENABLE 0

int CHANGE_AGTR_ENABLE = 0;
int LAYER_CONTROL;

float TRAINING_TIME;
// float TRAINING_TIME = 10; // COMP
// float TRAINING_TIME = 2.5; // COMM


/* Should be determined according to your network environment */
#define PORT_ID 1
#define NUM_WORKER 3	
#define WORKER_ID 1		
...
// #define APP_ID 0x2 // 32bit job2



// #include <window_manager.h>
#define TX_PACKET_LENGTH 305
#define MAX_AGTR_SIZE 1000 // Actually 2*MAX_AGTR_SIZE used in switch because we use two entries
...
// #define TIMEOUT_NS 1000000000 // 1s,
// #define TIMEOUT_NS 100000000 // 100ms,
// #define TIMEOUT_NS 1000000 // 1ms,
// #define TIMEOUT_NS 200000 // 200us(atp)
// #define TIMEOUT_NS 100000 // 100us
...
uint64_t total_dup_error_sent_packet = 0;
						...
						struct window_manager *window_manager, struct rte_mempool *tx_mbuf_pool)
{
	// if (num_pkts_to_send <= TX_BURST_SIZE){
		...
		if(window_manager->isSent[seq_num] != 0 && resend_flag == 0){
			RTE_LOG(WARNING, WORKER, "ERROR:DUP SENT %d pkt (f: %d) \n", seq_num, pkt_ceina_hdr.flags);
			total_dup_error_sent_packet++;
...
int Reset(uint32_t packet_total, struct window_manager *window_manager)
{
	...
	memset(window_manager->cnt, 0, sizeof(uint8_t) * (packet_total+1)); ///
...
int main(int argc, char **argv) 
{
	...
	uint8_t	quan_model_layer_jobid = quan_level + model_size + layer_index + (APP_ID & 0xF); 
	...
	int num_received_dup_error_pkts = 0;
	...
	uint64_t total_dup_error_packet = 0;
	// uint64_t total_dup_error_sent_packet = 0;
	...
	while ((c = getopt(argc, argv, "m:s:d:hf:c:")) != -1)
		switch (c)
		{
		case 'm':
		...
		case 'f':
            ...
            RTE_LOG(WARNING, WORKER, "FILE_NAME_SUFFIX: %s \n", FILE_NAME_SUFFIX);
            break;
		case 'c':
			if( strcmp(optarg, "COMP1") == 0 ){
				// CHANGE_AGTR_ENABLE = 1;
				LAYER_CONTROL = 0;
				TRAINING_TIME = 10;
				RTE_LOG(WARNING, WORKER, "COMP func1, LAYER_CONTROL=%d \n", LAYER_CONTROL);
			}
			else if( strcmp(optarg, "COMP12") == 0 ){
				CHANGE_AGTR_ENABLE = 0;
				LAYER_CONTROL = 1;
				TRAINING_TIME = 10;
				RTE_LOG(WARNING, WORKER, "COMP func12, LAYER_CONTROL=%d \n", LAYER_CONTROL);
			}
			else if( strcmp(optarg, "COMM1") == 0 ){
				// CHANGE_AGTR_ENABLE = 0;
				LAYER_CONTROL = 0;
				TRAINING_TIME = 2.5;
				RTE_LOG(WARNING, WORKER, "COMM func1, LAYER_CONTROL=%d \n", LAYER_CONTROL);
			}
			else if( strcmp(optarg, "COMM12") == 0 ){
				CHANGE_AGTR_ENABLE = 0;
				LAYER_CONTROL = 1;
				TRAINING_TIME = 2.5;
				RTE_LOG(WARNING, WORKER, "COMM func12, LAYER_CONTROL=%d \n", LAYER_CONTROL);
			}
            break;
		}
	...
	window_manager.cnt = (uint8_t *)calloc( (TOTAL_PACKET+1), sizeof(uint8_t));
	...
	for(int round=1; round <= NUM_ROUND; round++){
		...
		for (int i=0; i<TOTAL_PACKET; i++){
			if (window_manager.isACKed[i] != 0){
				isacked_cnt++;
			}
		}
		RTE_LOG(WARNING, WORKER, "isacked_cnt : %d \n", isacked_cnt);
			...
			for (uint16_t j = 0; j < nb_rx; ++j)
			{
				...
				if (window_manager.isACKed[seq_num] == 1)
				{
					if (is_resend_packet == true){
						...
						RTE_LOG(WARNING, WORKER, "flags %d, bitmap %d, PAS %d  \n", flags, bitmap, rte_cpu_to_be_32(ceina_hdr->PAS));
						num_received_dup_error_pkts++;
						total_dup_error_packet++;
					}
				}
				...
				{ ///
					...
					while (is_pending_queue_empty() == 0)
					{
						...
						// N.2.B 보낼 pkt의 seq_num이 보낼 수 있는 범위에 있다.
						...
						{ // 보낼 필요가 없음
							...
							// printf("pkt %d : no need to send packet from pending queue \n", next_seq_num);
						}
					}
					...
					{ // todo
						if (seq_num >= finish_window_seq)
						{
							...
							int new_window = adjustWindow(&cc_manager, is_ecn_mark_packet, 0);
							if (send_pointer + new_window > window_manager.total_ACK)
							{
								window = window_manager.total_ACK - send_pointer;
							}
		...
		RTE_LOG(WARNING, WORKER, "num_received_dup_error_pkts      : %d \n", num_received_dup_error_pkts);
		...
		RTE_LOG(WARNING, WORKER, "l1_loss_cnt %d \n", l1_loss_cnt);	
		...
		RTE_LOG(WARNING, WORKER, "total_dup_error_packet 	: %llu \n", total_dup_error_packet);
		...
		RTE_LOG(WARNING, WORKER, "total_dup_err_sent_pkt	: %llu \n", total_dup_error_sent_packet);
    ...
    fprintf(file_ptr, "total_dup_packet    		 : %d \n", total_dup_packet);
    fprintf(file_ptr, "total_dup_error_packet    : %d \n", total_dup_error_packet);
