...
p4 = bfrt.ceina_cont1.pipe.Ingress
...
for k in range(num_app):
    ...
    ## isAggregate=0이면 aggregation을 완료시킨 것이 아니므로 (?)
    for j in range(1, len_PS[k]):
        print("outPort1-2")
        p4.outPort_table.add_with_nop(
            PAS=PAS[k], PAS_mask=PAS_mask,
            ingress_port= single_loopback_port,
            dataIndex=1,
            PSIndex=j-1,
...
p4_egress = bfrt.ceina_cont1.pipe.Egress
