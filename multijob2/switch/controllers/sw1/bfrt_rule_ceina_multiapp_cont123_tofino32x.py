...
p4 = bfrt.ceina_cont123.pipe.Ingress
...
# Job1
port_of_worker.append([0, 168, 169, 136, 171]) # host5 host6 host7(tofino65x2) host8
PSs.append ([0,4]) 
...
p4.forward.add_with_set_egr( # global
        ethernet_dstAddr=0x000000000002,
        ethernet_dstAddr_mask=0xFFFFFFFFFFFF,
        ipv4_dstAddr=0,
        ipv4_dstAddr_mask=0,
        ipv4_protocol = 0,
        ipv4_protocol_mask = 0xFF,
        p4ml_isACK = 1,
        p4ml_isACK_mask = 1,
        
        egress_spec=184, # gpu1
        qid=2
)
...
for k in range(num_app):
    ...
    for j in range(1, len_PS[k]):
        p4.outPort_table2.add_with_set_egr_and_set_index_queue(
            PAS=PAS[k], PAS_mask=PAS_mask,
            ingress_port=single_loopback_port,
            dataIndex=1, # 
            PSIndex=j-1,
    ...
    for j in range(1, len_PS[k]):
        ...
        p4.multicast_table.add_with_multicast(
            ...
            # hdr.p4ml.PAS mask 0xFFFF0000: exact;
...
p4_egress = bfrt.ceina_cont123.pipe.Egress
...
p4_egress.ttl_table.add_with_nop(
        egress_port=196,
        isACK=0
)
p4_egress.ttl_table.add_with_nop(
...
p4_egress.ttl_table.add_with_allocate_ttl(
        egress_port=136,
        isACK=1,
        
        ttl_value=255 # ACK for host7
)
