...
#define DST_MAC_ADDR "00:00:00:00:00:01" // job1:1 / job2:2
#define IP_DEFTTL 0 
...
void send_packet_in_burst(){
	...
	// uint16_t nb_tx = rte_eth_tx_burst(PORT_ID, 0, pkts_tx_burst, num_pkts_to_send);
	// if (nb_tx < num_pkts_to_send) {
	// 	RTE_LOG(NOTICE, PS, "Failed to transmit all packets, nb_tx:%d, num_pkts_to_send:%d \n", nb_tx, num_pkts_to_send);
	// 	for (int i = 0; i < num_pkts_to_send; i++) {
	// 		rte_pktmbuf_free(pkts_tx_burst[i]);
	// 	}
	// }
	// else if (nb_tx != 0 ){
	// 	RTE_LOG(INFO, PS, "Sent %d (nb_tx) packets \n", nb_tx);
	// }
	...
	// }
	// num_pkts_to_send = 0; // 전송한 패킷 수 초기화
...
int inline Reset(int packet_total, struct window_manager *window_manager)
{
	...
	// window_manager->isACKed = (bool*)calloc(packet_total, sizeof(bool));
...
{
	// rte_log_register("ps_log");
    // rte_log_set_level(MY_LOG_CATEGORY, RTE_LOG_INFO);
	rte_log_set_global_level(LOG_LEVEL);
	//RTE_LOG(INFO, MY_LOG_CATEGORY, "The value of variable is: %d\n", variable);
	...
	for (int i = 1; i <= MAX_APP_PER_THREAD; i++)
	{
		...
		// if (tensors_pos_of_app[i] == 1)
		// {
		// 	tensors_pos_of_app[i] = 0;
		// }
		printf("tensors_pos_of_app[%d]: %d \n", i, tensors_pos_of_app[i]);
	}

	// Creates queue to store packets to be sent : (related with window)
	// pendingQueue = new std::queue<agghdr *>[num_thread]; //todo:
	...
	/* 5. Setup window manager */ // todo
	// struct window_manager window_manager;
	// window_manager.total_ACK = TOTAL_PACKET;
	// window_manager.last_ACK = 0;
	// window_manager.isACKed = (bool *)malloc(sizeof(bool) * window_manager.total_ACK);
	// memset(window_manager.isACKed, 0, sizeof(bool) * window_manager.total_ACK);

	// struct rte_mbuf *pkts_rx_burst[64] = {0}; ///FIXME
	// struct rte_mbuf **pkts_rx_burst = (struct rte_mbuf **) rte_calloc_socket(NULL, MAX_WINDOW_SIZE, sizeof(struct rte_mbuf*), RTE_CACHE_LINE_SIZE, rte_socket_id());
	// if (pkts_rx_burst == NULL){
	// 	RTE_LOG(INFO, PS, "Cannot allocate pkts_rx_burst \n");
	// }
	...
	while (1)
	{
		...
		// for (int i = 0; i < 64; i++) {
		// 	if (pkts_rx_burst[i] != NULL) {
		// 		RTE_LOG(INFO, PS, "nb_rx: %d Index: %d, Value: %d\n", msgs_completed, i, pkts_rx_burst[i]);
		// 	}
		// }
		...
		for (int msg = 0; msg < msgs_completed; msg++)
		{
			...
			// bool isTerminated_packet = ceina_hdr->flags & 0x02;
			...
			// if (seq_num <= MAX_WINDOW_SIZE){
			// 	RTE_LOG(WARNING, PS, "Received: seq %d, app_id %d, bmap %d, agtr_time %d, agtr_idx %d, agtr_idx2 %d, f %d, level: %d, length: %d \n", 
			// 		seq_num, app_id, bitmap, agtr_time, agtr_idx, agtr_idx2, flags, quantization_flags, packet_length);
			// }
			...
			if(app_id <= 0x8){
				...
				// printf("ceina_entry1->data[0] data[31] : %d %d, ceina_entry2->data[0] data[31] : %d, %d \n", 
					// ceina_entry1->data[0], ceina_entry1->data[31], ceina_entry2->data[0], ceina_entry2->data[31]);
				// ceina_hdr->quantization_flags = ceina_hdr->quantization_flags & 0x05;
			}
			...
			// if (EANBLE_LOG){
			// 	printf("------------------------------------------------- \n");
			// 	printf("Received: seq_num %d, app_id %d, bitmap %d, agtr_idx %d, flags %d \n", seq_num, app_id, bitmap, agtr_idx, ceina_hdr->flags);
			// 	printf("src_addr %d, dst_addr %d, agtr_time %d \n", src_addr, dst_addr, agtr_time);
			// }
			...
			if ( is_collision_packet )
			{
				...
				// RTE_LOG(WARNING, PS, "isCollision[%d] is 1 /  agtr_idx: %d \n", seq_num, agtr_idx);
				// RTE_LOG(WARNING, PS, "Received: seq %d, app_id %d, bmap %d, agtr_time %d, agtr_idx %d, agtr_idx2 %d, f %d, level: %d, length: %d \n", 
				// 					seq_num, app_id, bitmap, agtr_time, agtr_idx, agtr_idx2, flags, quantization_flags, packet_length);				
				total_collision_pkts++;
				total_collision_pkt[appID]++;
				num_collision_pkt[appID]++;
				if(quantization_flags & 0x10){ 
					total_collision_by_preemption_pkt[appID]++;
				}
			...
			// if(seq_num == 1 & test_print_once[appID] == 0){
			// 	for(int i=0; i<3; i++){
			// 		RTE_LOG(WARNING, PS, "appID %d \n", appID);
					// RTE_LOG(WARNING, PS, "isACKed[%d] is %d \n", 10, tensors[my_tensors_pos].window_manager_arr[i].isACKed[10]);
					// RTE_LOG(WARNING, PS, "isACKed[%d] is %d \n", 100, tensors[my_tensors_pos].window_manager_arr[i].isACKed[100]);
					// RTE_LOG(WARNING, PS, "isACKed[%d] is %d \n", 200, tensors[my_tensors_pos].window_manager_arr[i].isACKed[200]);
					// RTE_LOG(WARNING, PS, "isACKed[%d] is %d \n", 1000, tensors[my_tensors_pos].window_manager_arr[i].isACKed[1000]);
					// RTE_LOG(WARNING, PS, "isACKed[%d] is %d \n", 10000, tensors[my_tensors_pos].window_manager_arr[i].isACKed[10000]);
				// }
			// 	test_print_once[appID]++;
			// }
			// if (OVERFLOW_HADLE) //todo
			...
			else
			{
				...
				// // TEST
				// if(previous_full_pkt[appID] == total_full_pkt[appID]){
				// 	count_test[appID]++;
				// 	if (count_test[appID] % 1000 == 0){
				// 		check[appID] = true;
				...
				// }
				// previous_full_pkt[appID] = total_full_pkt[appID];
					...
					if (need_to_update)
					{
						...
						for (int i = 0; i < num_worker; i++)
						{
							if (valid_bit & bitmap)
							{
								...
								// if (EANBLE_LOG)
								// 	printf("worker %d UpdateWindow completed \n", i);
							}
							valid_bit <<= 1;
						}
			...
			if (is_slot_completed)
			{
				...
				// ceina_hdr->bitmap = rte_cpu_to_be_32(1);
				uint16_t new_agtr;
				...
				if(CHANGE_AGTR_ENABLE){
					if (tensors[my_tensors_pos].isCollision[seq_num] == true) {
						...
						// p4ml_header_setLengthFieldToAgtr(p4ml_header, new_agtr); //todo
					}
					else{
						// p4ml_header_resetCollisionBit(p4ml_header); //todo
						// new_bitmap = 7;
						flags = flags & ~(1 << 1);
					}
				}
				...
				// p4ml_header_hton_without_data(p4ml_header); //todo
				...
				// 1-2-10-5 worker 별로 tensor aggregation이 모두 완료되었는지 확인
				...
				// 	{
				// 		if (tensors[tensors_pos_of_app[appID]].window_manager_arr[i].last_ACK < tensors[tensors_pos_of_app[appID]].window_manager_arr[i].total_ACK)
				// 		{
		...
		if (msgs_completed > 0)
		{
			...
			// if (receive_byte_reset_flag)
			// {
			// 	receive_in_sec[appID] = msgs_completed;
			// 	receive_byte_reset_flag[appID] = false;
			// }
			// else
			// 	receive_in_sec[appID] += msgs_completed;
		...
		// RTE_LOG(WARNING, PS, "time_span_end_file: %llu \n", time_span_end_file);
		if(time_span_end_file >= 2000000000L){ // 10s
